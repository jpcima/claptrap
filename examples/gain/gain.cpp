...
bool gain__gui_create(const clap_plugin_t *plugin, const char *api, bool is_floating)
{
    ...
    SharedPointer<CTextButton> button = makeOwned<CTextButton>(
        CRect{0, 0, 100, 100}, nullptr, -1, "Hello, World!");
...
//------------------------------------------------------------------------------
#if !defined(_WIN32) && !defined(__APPLE__)
static void gain__posix_fd_support_on_fd(const clap_plugin_t *plugin, int fd, int flags)
{
    gain__private_t *priv = (gain__private_t *)plugin->plugin_data;
