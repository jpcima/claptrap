...
public:
    explicit MyX11TimerSupport(const clap_host *host);
...
private:
    ...
    const clap_host_timer_support *m_support = nullptr;
...
MyX11TimerSupport::MyX11TimerSupport(const clap_host *host)
    : m_host{host}
{
    m_support = (const clap_host_timer_support *)host->get_extension(host, CLAP_EXT_TIMER_SUPPORT);
}
...
bool MyX11TimerSupport::registerTimer(uint64_t interval, VSTGUI::X11::ITimerHandler *handler)
{
    ...
    const clap_host *host = m_host;
    const clap_host_timer_support *support = m_support;
...
{
public:
    explicit MyX11FdSupport(const clap_host *host);
...
private:
    const clap_host *m_host = nullptr;
    const clap_host_posix_fd_support *m_support = nullptr;
...
MyX11FdSupport::MyX11FdSupport(const clap_host *host)
    : m_host{host}
{
    m_support = (const clap_host_posix_fd_support *)host->get_extension(host, CLAP_EXT_POSIX_FD_SUPPORT);
}
...
bool MyX11FdSupport::registerEventHandler(int fd, VSTGUI::X11::IEventHandler *handler)
{
    ...
    const clap_host *host = m_host;
    const clap_host_posix_fd_support *support = m_support;
...
{
public:
    explicit MyX11RunLoop(const clap_host *host);
...
MyX11RunLoop::MyX11RunLoop(const clap_host *host)
...
//------------------------------------------------------------------------------
VSTGUI::X11::IRunLoop *new_vstgui_x11_runloop(const clap_host *host)
{
