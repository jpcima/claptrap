...
static void invoke_posix_fd_callback(ct_host::posix_fd_data *posix_fd_data, int fd, int flags)
{
    ...
    const clap_plugin_posix_fd_support_t *posix_fd_support = comp->m_ext.m_posix_fd_support;
    if (posix_fd_support)
        posix_fd_support->on_fd(plug, fd, flags);
}

bool ct_host::posix_fd__register_fd(const clap_host_t *host, int fd, int flags)
{
    ct_component *comp = (ct_component *)host->host_data;
    ct_host *self = comp->m_host.get();
...
bool ct_host::posix_fd__modify_fd(const clap_host_t *host, int fd, int flags)
{
    (void)host;
    (void)fd;
    (void)flags;
...
bool ct_host::posix_fd__unregister_fd(const clap_host_t *host, int fd)
{
    ct_component *comp = (ct_component *)host->host_data;
    ct_host *self = comp->m_host.get();
