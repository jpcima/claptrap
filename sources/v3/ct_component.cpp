...
ct_component::ct_component(const v3_tuid clsiid, const clap_plugin_factory *factory, const clap_plugin_descriptor *desc, v3::object *hostcontext, bool *init_ok)
{
    *init_ok = false;
    ...
    const clap_plugin *plug = factory->create_plugin(factory, &host->m_clap_host, desc->id);
    if (!plug)
        return;
    ...
    // extensions
    const clap_plugin_audio_ports *audio_ports = (const clap_plugin_audio_ports *)plug->get_extension(plug, CLAP_EXT_AUDIO_PORTS);
    if (!audio_ports)
        audio_ports = get_default_clap_plugin_audio_ports();
    m_ext.m_audio_ports = audio_ports;

    const clap_plugin_audio_ports_config *audio_ports_config = (const clap_plugin_audio_ports_config *)plug->get_extension(plug, CLAP_EXT_AUDIO_PORTS_CONFIG);
    m_ext.m_audio_ports_config = audio_ports_config;

    const clap_plugin_surround *surround = (const clap_plugin_surround *)plug->get_extension(plug, CLAP_EXT_SURROUND);
    m_ext.m_surround = surround;

    const clap_plugin_state *state = (const clap_plugin_state *)plug->get_extension(plug, CLAP_EXT_STATE);
    m_ext.m_state = state;

    const clap_plugin_latency *latency = (const clap_plugin_latency *)plug->get_extension(plug, CLAP_EXT_LATENCY);
    m_ext.m_latency = latency;

    const clap_plugin_tail *tail = (const clap_plugin_tail *)plug->get_extension(plug, CLAP_EXT_TAIL);
    m_ext.m_tail = tail;

    const clap_plugin_render *render = (const clap_plugin_render *)plug->get_extension(plug, CLAP_EXT_RENDER);
    m_ext.m_render = render;

    const clap_plugin_params *params = (const clap_plugin_params *)plug->get_extension(plug, CLAP_EXT_PARAMS);
    m_ext.m_params = params;

    const clap_plugin_timer_support *timer_support = (const clap_plugin_timer_support *)plug->get_extension(plug, CLAP_EXT_TIMER_SUPPORT);
    m_ext.m_timer_support = timer_support;

#if CT_X11
    const clap_plugin_posix_fd_support *posix_fd_support = (const clap_plugin_posix_fd_support *)plug->get_extension(plug, CLAP_EXT_POSIX_FD_SUPPORT);
    m_ext.m_posix_fd_support = posix_fd_support;
#endif

    const clap_plugin_gui *gui = (const clap_plugin_gui *)plug->get_extension(plug, CLAP_EXT_GUI);
    m_ext.m_gui = gui;
...
static void cache_audio_ports_internal(
    ...
    bool is_input, std::vector<clap_audio_port_info> &result)
{
    result.clear();
...
void ct_component::cache_audio_port_configs()
{
    ...
    const clap_plugin_audio_ports_config *audio_ports_config = m_ext.m_audio_ports_config;
...
static void cache_channel_map_internal(
    const clap_plugin *plug, const clap_plugin_surround *surround,
    nonstd::span<const clap_audio_port_info> ports,
...
void ct_component::cache_params()
{
    ...
    std::vector<clap_param_info> &result = m_cache.m_params;
...
const clap_param_info *ct_component::get_param_by_id(clap_id id) const
{
...
int32_t V3_API ct_component::get_bus_count(void *self_, int32_t media_type, int32_t bus_direction)
{
    ...
    if (media_type == V3_AUDIO) {
        const std::vector<clap_audio_port_info> &ports = (bus_direction == V3_INPUT) ?
