...
const clap_plugin_audio_ports_t *get_default_clap_plugin_audio_ports()
{
   static auto count = [](const clap_plugin_t *plugin, bool is_input) -> uint32_t
   {
       (void)plugin;
       (void)is_input;
       return 1;
   ...
   static auto get = [](const clap_plugin_t *plugin, uint32_t index, bool is_input, clap_audio_port_info_t *info) -> bool
   {
       (void)plugin;
       if (index != 0)
           return false;
...
bool convert_to_clap_window(void *wnd, nonstd::string_view platform_type, clap_window_t *window)
{
#if defined(_WIN32)
