...
struct ct_component {
    ct_component(const v3_tuid clsiid, const clap_plugin_factory_t *factory, const clap_plugin_descriptor_t *desc, v3::object *hostcontext, bool *init_ok);
    ~ct_component();
    ...
    void cache_params();
    const clap_param_info_t *get_param_by_id(clap_id id) const;
    ...
    const clap_plugin_descriptor_t *m_desc = nullptr;
    ...
    // extensions
    struct {
        const clap_plugin_audio_ports_t *m_audio_ports = nullptr; // required
        const clap_plugin_audio_ports_config_t *m_audio_ports_config = nullptr; // optional
        const clap_plugin_surround_t *m_surround = nullptr; // optional
        const clap_plugin_state_t *m_state = nullptr; // optional
        const clap_plugin_latency_t *m_latency = nullptr; // optional
        const clap_plugin_tail_t *m_tail = nullptr; // optional
        const clap_plugin_render_t *m_render = nullptr; // optional
        const clap_plugin_params_t *m_params = nullptr; // optional
        const clap_plugin_timer_support_t *m_timer_support = nullptr; // optional
#if CT_X11
        const clap_plugin_posix_fd_support_t *m_posix_fd_support = nullptr; // optional
#endif
        const clap_plugin_gui_t *m_gui = nullptr; // optional
    } m_ext;

    // caches
    struct ports_config_t {
        clap_audio_ports_config_t m_config;
        std::vector<clap_audio_port_info_t> m_inputs;
        std::vector<clap_audio_port_info_t> m_outputs;
    };

    struct {
        std::vector<ports_config_t> m_audio_ports_config;
        std::vector<clap_audio_port_info_t> m_audio_inputs;
        std::vector<clap_audio_port_info_t> m_audio_outputs;
        std::vector<std::vector<uint8_t>> m_input_channel_maps;
        std::vector<std::vector<uint8_t>> m_output_channel_maps;
        std::vector<clap_param_info_t> m_params;
