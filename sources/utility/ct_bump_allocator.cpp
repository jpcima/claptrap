...
struct bump_allocator::descriptor {
    std::size_t size;
...
bump_allocator::bump_allocator(bump_allocator &&o) noexcept
...
bump_allocator::bump_allocator(std::uint8_t *data, std::size_t cap, std::size_t al) noexcept
    : data_(data), cap_(cap), mask_(al - 1)
{
}

bump_allocator &bump_allocator::operator=(bump_allocator &&o) noexcept
{
    data_ = o.data_;
    top_ = o.top_;
    cap_  = o.cap_;
...
std::size_t bump_allocator::pad_size(std::size_t size) const noexcept
{
    return (size + mask_) & ~mask_;
}

void *bump_allocator::alloc(std::size_t size) noexcept
{
...
void *bump_allocator::unchecked_alloc(std::size_t size) noexcept
{
    std::size_t totalsize = pad_size(size + sizeof(descriptor));
...
void bump_allocator::free(const void *ptr) noexcept
{
    CT_ASSERT(ptr);
    (void)ptr;
